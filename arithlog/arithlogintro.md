...
1. Condition Flags (N, Z, C, V): Diese Flags befinden sich in den oberen 4 Bits (Bits 31 bis 28) des Registers. Sie werden von arithmetischen und logischen Operationen beeinflusst und helfen dabei, den Zustand des Ergebnisses einer Berechnung zu bestimmen.
    ...
    - **C (Carry Flag)**: Das Carry-Flag zeigt an, ob bei einer arithmetischen Operation ein Übertrag oder Untertrag (Über- oder Unterschreitung des Zahlenbereichs) auftritt, wenn man Zahlen als unvorzeichenbehaftet (unsigned) betrachtet.
...
2. Weitere Flags und Steuerbits: Zusätzlich zu den Condition Flags enthält das CPSR noch weitere Flags
   > Weitere Flags absprechen
...
Bei Subtraktionen zeigt das Carry-Flag an, dass kein Borrow (kein Unterlauf) aufgetreten ist. Das bedeutet, wenn die Subtraktion erfolgreich ohne Borrow durchgeführt wird, bleibt das Carry-Flag gesetzt (C = 1).
